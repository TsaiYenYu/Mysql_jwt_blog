...
func InitRouter() {
	...
	*/
	...
	auth.Use(middleware.JwtToken())
	{
		...
		//修改預約地點
		auth.POST("location/add", v1.AddLocation)
		auth.PUT("location/:id", v1.EditLocation)

		//取得活動資料
		auth.POST("event/add", v1.AddEvent)
		auth.PUT("event/:id", v1.EditEvent)
	}
	...
	*/
	router := r.Group("api/v1")
	{
		...
		//取得地點資料
		router.GET("location/:id", v1.GetLocationInfo)
		router.GET("location", v1.GetLocation)
